...
Wraps `@tanarchy/lotl`, an on-device hybrid search + memory framework with
...
Dependencies
------------
- `qmd` binary on PATH (or `LOTL_BINARY` env var pointing at it).
  Install: `npm install -g @tanarchy/lotl` or build from source.
- Node.js >= 22.
...
----------
    ...
    provider_l1 = QmdMemoryProvider(
        name_suffix="l1",
        env_overrides={"LOTL_INGEST_USER_ONLY": "on"},
    )
...
_DEFAULT_LOTL_ENV: dict[str, str] = {
...
class QmdMemoryProvider(MemoryProvider):
    ...
    def __init__(
        ...
        self._binary = binary or os.environ.get("LOTL_BINARY") or shutil.which("qmd")
        if not self._binary:
            raise RuntimeError(
                "qmd binary not found. Install via `npm install -g @tanarchy/lotl` "
                "or set LOTL_BINARY=/path/to/qmd."
            )
    ...
    def initialize(self) -> None:
        ...
        # — NOT the LOTL_CACHE_DIR env var, which doesn't exist in qmd. Without
        # this, all 3 configs in a sweep would write to ~/.cache/lotl/index.sqlite
    ...
    def prepare(self, store_dir: Path, unit_ids: set[str] | None = None, reset: bool = True) -> None:
        # qmd's storage is managed by initialize() via LOTL_CACHE_DIR; nothing
    ...
    def ingest(self, documents: list[Document]) -> None:
        ...
        plain strings).
        """
        user_only = self._env_overrides.get("LOTL_INGEST_USER_ONLY") == "on"
