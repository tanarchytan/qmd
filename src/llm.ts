...
// LLM interface moved to src/llm/types.ts alongside ILLMSession.
// =============================================================================
...
// Generic session subsystem moved to src/llm/session.ts. Re-export for
// external consumers that import from "./llm.js".
export { LLMSessionManager, LLMSession, SessionReleasedError, withLLMSessionForLlm } from "./llm/session.js";

import { LLMSessionManager as _LLMSessionManagerImpl } from "./llm/session.js";
type _LLMSessionManager = InstanceType<typeof _LLMSessionManagerImpl>;

// Session manager for the default LlamaCpp instance (stays here because
// it binds to the LlamaCpp singleton that still lives in this file).
let defaultSessionManager: _LLMSessionManager | null = null;

/**
 * Get the session manager for the default LlamaCpp instance.
 */
function getSessionManager(): _LLMSessionManager {
  const llm = getDefaultLlamaCpp();
  if (!defaultSessionManager || defaultSessionManager.getLlm() !== llm) {
    defaultSessionManager = new _LLMSessionManagerImpl(llm);
  }
...
export async function withLLMSession<T>(
  ...
  const { LLMSession: _LLMSession } = await import("./llm/session.js");
  const session = new _LLMSession(manager, options);
...
// =============================================================================
// Remote LLM (Cloud Providers) — implementation moved to src/llm/remote.ts
// =============================================================================
