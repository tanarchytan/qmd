#!/usr/bin/env bash
# setup-qmd.sh — One-click installer for QMD (Search + Memory + Knowledge Graph)
# Usage: curl -fsSL https://raw.githubusercontent.com/tanarchytan/qmd/dev/setup/setup-qmd.sh | bash
#   or:  bash setup-qmd.sh [--dry-run] [--uninstall] [--provider <name>] [--local]
set -euo pipefail

VERSION="1.0.0"
PACKAGE="@tanarchy/qmd@dev"
PLUGIN_ID="tanarchy-qmd"

# Colors
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; BLUE='\033[0;34m'; NC='\033[0m'

info()  { printf "${BLUE}[qmd]${NC} %s\n" "$*"; }
ok()    { printf "${GREEN}[qmd]${NC} %s\n" "$*"; }
warn()  { printf "${YELLOW}[qmd]${NC} %s\n" "$*"; }
err()   { printf "${RED}[qmd]${NC} %s\n" "$*" >&2; }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      echo "setup-qmd.sh v${VERSION} — QMD installer"
      echo ""
      echo "Usage: bash setup-qmd.sh [options]"
      echo ""
      echo "Options:"
...
if $UNINSTALL; then
  ...
  npm uninstall -g @tanarchy/qmd 2>/dev/null || true
  ok "QMD uninstalled"
  exit 0
fi
...
if $DRY_RUN; then
  ...
  if $HAS_OPENCLAW; then
    ...
    info "[dry-run] Would write env config to ~/.config/qmd/.env"
  fi
...
case "$PROVIDER" in
  ...
  nebius)
    ...
    ENV_VARS=(
      ...
      [QMD_RERANK_API_KEY]="$ZE_KEY"
      [QMD_RERANK_MODEL]="zerank-2"
      [QMD_QUERY_EXPANSION_PROVIDER]="api"
      [QMD_QUERY_EXPANSION_API_KEY]="$NEB_KEY"
  ...
  openai)
    ...
    ENV_VARS=(
      ...
      [QMD_EMBED_DIMENSIONS]="1536"
      [QMD_QUERY_EXPANSION_PROVIDER]="openai"
      [QMD_QUERY_EXPANSION_API_KEY]="$OAI_KEY"
      [QMD_QUERY_EXPANSION_MODEL]="gpt-4o-mini"
    )
    ;;
  local)
...
if [[ "$PROVIDER" != "local" ]]; then
  ...
  # Quick embed probe
  EMBED_URL="${ENV_VARS[QMD_EMBED_URL]:-}"
  EMBED_PROVIDER="${ENV_VARS[QMD_EMBED_PROVIDER]}"
  EMBED_KEY="${ENV_VARS[QMD_EMBED_API_KEY]}"
  EMBED_MODEL="${ENV_VARS[QMD_EMBED_MODEL]}"
...
if $HAS_OPENCLAW; then
  ...
  build_plugin_config() {
    ...
    if [[ "$PROVIDER" == "local" ]]; then
      ...
      # Embed
      if [[ -n "${ENV_VARS[QMD_EMBED_PROVIDER]:-}" ]]; then
        cfg=$(echo "$cfg" | jq \
          --arg provider "${ENV_VARS[QMD_EMBED_PROVIDER]}" \
          --arg apiKey "${ENV_VARS[QMD_EMBED_API_KEY]:-}" \
          --arg url "${ENV_VARS[QMD_EMBED_URL]:-}" \
          --arg model "${ENV_VARS[QMD_EMBED_MODEL]:-}" \
          --arg dimensions "${ENV_VARS[QMD_EMBED_DIMENSIONS]:-}" \
      ...
      # Rerank
      if [[ -n "${ENV_VARS[QMD_RERANK_PROVIDER]:-}" ]]; then
        cfg=$(echo "$cfg" | jq \
          --arg provider "${ENV_VARS[QMD_RERANK_PROVIDER]}" \
          --arg apiKey "${ENV_VARS[QMD_RERANK_API_KEY]:-}" \
          --arg url "${ENV_VARS[QMD_RERANK_URL]:-}" \
          --arg model "${ENV_VARS[QMD_RERANK_MODEL]:-}" \
          --arg mode "${ENV_VARS[QMD_RERANK_MODE]:-}" \
      ...
      # Query expansion
      if [[ -n "${ENV_VARS[QMD_QUERY_EXPANSION_PROVIDER]:-}" ]]; then
        cfg=$(echo "$cfg" | jq \
          --arg provider "${ENV_VARS[QMD_QUERY_EXPANSION_PROVIDER]}" \
          --arg apiKey "${ENV_VARS[QMD_QUERY_EXPANSION_API_KEY]:-}" \
          --arg url "${ENV_VARS[QMD_QUERY_EXPANSION_URL]:-}" \
          --arg model "${ENV_VARS[QMD_QUERY_EXPANSION_MODEL]:-}" \
  ...
  QMD_CONFIG_DIR="${QMD_CONFIG_DIR:-${XDG_CONFIG_HOME:-$HOME/.config}/qmd}"
  mkdir -p "$QMD_CONFIG_DIR"
  ENV_FILE="$QMD_CONFIG_DIR/.env"
  ...
  {
    echo "# QMD configuration — generated by setup-qmd.sh v${VERSION}"
    echo "# $(date -Iseconds)"
    echo ""
...
if command -v qmd &>/dev/null; then
  ...
  qmd status 2>/dev/null && ok "QMD is ready ✓" || warn "qmd status returned non-zero (may need collections)"
fi
...
echo ""
info "Next steps:"
echo "  1. Add content:  qmd collection add ~/path/to/markdown --name myknowledge"
echo "  2. Embed:        qmd embed"
echo "  3. Search:       qmd query 'your question here'"
if $HAS_OPENCLAW; then
