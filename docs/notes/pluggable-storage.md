...
- Filesystem-level workflows: `cp ~/.cache/qmd/index.sqlite backup/`, open
...
```sh
QMD_STORAGE_BACKEND=sqlite-vec    # default, on-device
QMD_STORAGE_BACKEND=pgvector      # PG_URL=postgres://...
QMD_STORAGE_BACKEND=qdrant        # QDRANT_URL=http://...
```
