...
**TL;DR:** Confirmed the 82% multi-session R@5 ceiling is breakable — two independent levers moved it tonight. `snowflake-arctic-embed-s q8` (a direct model swap) hit **84.2% multi-session** without any code change — the first time any embed model broke 83% on LME _s n=500. Code-side, `LOTL_MEMORY_EXPAND=keywords` zero-LLM fanout lifted mxbai-xs q8 multi-session from 82% to 83%. Both levers are independent and stack. Queued for tomorrow: n=500 arctic-s × levers shootout to see if they combine superadditively.
...
- **Per-turn ingest A/B on LME** — currently `LOTL_INGEST_PER_TURN=off`. Turning on increases memories per scope from ~50 to ~500 (finer granularity). High-potential multi-session lever. Untested.
...
- `LOTL_VEC_FLOOR_RATIO` env knob — per-model adaptive floor calibration. mxbai-xs q8 needs a lower ratio than MiniLM-L6 fp32. Avoids the "tight distribution → empty pool" problem that killed MMR on mxbai-xs.
...
- `LOTL_MEMORY_EXPAND=keywords` promotion path — if confirmed on arctic-s at phase 4, promote to default on LME-class workloads.

### Shippable flags (all off-by-default, opt-in)

| Flag | Feature | Signal |
|---|---|---|
| `LOTL_EMBED_BACKEND=transformers` + `LOTL_TRANSFORMERS_MODEL=Snowflake/snowflake-arctic-embed-s` | arctic-s embed model | **+2.2pp multi-session** on LME n=500 |
| `LOTL_MEMORY_EXPAND=keywords` | Zero-LLM keyword-group query fanout | **+1pp multi-session** on LME n=500 |
| `LOTL_MEMORY_EXPAND=entities` | Zero-LLM proper-noun query fanout | 0pp on LME (sparse entities), useful on other workloads |
| `LOTL_MEMORY_MMR=session` | Dialog-diversity reshuffle in RAW mode | TBD (phase 4 fix) |
| `LOTL_MEMORY_SCOPE_NORM=rank` | Rank-normalize similarities within scope | Noop on LME (single-scope), useful on multi-project |
| `LOTL_VEC_MIN_SIM=0.1` | Override adaptive cosine floor | +0.8pp R@10 on mxbai-xs q8 n=500 |
...
4. **Model-aware floor ratio** — stop `LOTL_VEC_MIN_SIM=0.1` being a magic number. Auto-calibrate per embed model based on observed top-1 distribution.
