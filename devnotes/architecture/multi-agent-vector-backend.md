...
- How does `~/.cache/lotl/lancedb/` coexist with sqlite in `~/.cache/lotl/`
...
- Windows + macOS + Linux
- `~/.cache/lotl/` as the single cache root
...
**Storage model:** File-based (Lance/Arrow columnar format). Would coexist fine at `~/.cache/lotl/lancedb/` next to your SQLite file.
...
qmd avoids all three (we use `~/.cache/lotl/index.sqlite` always,
...
**Config shape** (mirrors `~/.config/lotl/.env`):
```
LOTL_VECTOR_BACKEND=sqlite-vec          # or lancedb, vectorlite
LOTL_VECTOR_PATH=~/.cache/lotl/vectors/  # backend-specific storage dir
```
...
@tanarchy/lotl`) really is a one-liner.
...
| KG in recall | No | Yes (LOTL_MEMORY_KG=on) |
| Temporal retrieval | No | Yes (3rd RRF list) |
| Push Pack API | No | Yes |
| Tiered recall API | No | Yes |
| Install | Multi-step (openclaw + peer dep) | One-liner (`npm install @tanarchy/lotl`) |
...
4. **Config surface as YAML.** `~/.config/lotl/config.yaml` alongside
...
5. **One-liner install.** Our `npm install @tanarchy/lotl` really works;
